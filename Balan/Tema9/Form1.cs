...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        //Alegere mod de randare quads cub 1.culori basic 2.prima textura 3 a doua textura
        private int renderModeCubQuads = 0;

        //Alegere mod de randare triangles cub 1.culori basic 2.prima textura 3 a doua textura
        private int renderModeCubTriangles = 0;
        ...
        //    GL.End();
        //}
        //Desenează cubul - quads cu texturi
        private void DeseneazaCubQ()
        {
            if (renderModeCubQuads == 0) {
                GL.Begin(PrimitiveType.Quads);
                for (int i = 0; i < nQuadsList; i++)
                {
                    switch (i % 4)
                ...
                GL.End();
                renderModeCubQuads += 1;
                return;
            }
            else if (renderModeCubQuads == 1)
            {
                GL.BindTexture(TextureTarget.Texture2D, textures[0]); // Alegerea primei texturi
                GL.Begin(PrimitiveType.Quads);
                GL.Color3(Color.White);
                addTextureToQuadsCube();
                GL.End();
                renderModeCubQuads += 1;
                return;
            }
            ...
            {
                GL.BindTexture(TextureTarget.Texture2D, textures[1]); // Alegerea celei de a doua texturi
                GL.Begin(PrimitiveType.Quads);
                GL.Color3(Color.White);
                addTextureToQuadsCube();
                GL.End();
                renderModeCubQuads = 0;
                return;
            }
        }
        private void addTextureToQuadsCube()
        {
            for (int i = 0; i < nQuadsList; i += 4)
            {
                // Coordonatele texturii și vertexurile pentru fiecare colț al quadului
        ...
        private void DeseneazaCubT() {
            if (renderModeCubTriangles == 0)
            {
                ...
                GL.End();
                renderModeCubTriangles++;
                return;
            }
            else if(renderModeCubTriangles == 1)
            {
                GL.BindTexture(TextureTarget.Texture2D, textures[0]); // Alegerea primei texturi
                GL.Begin(PrimitiveType.Triangles);
                GL.Color3(Color.White);
                addTextureToCubeTriangles();
                GL.End();
                renderModeCubTriangles++;
                return;
            }
            else
            {
                ...
                addTextureToCubeTriangles();
                GL.End();
                renderModeCubTriangles=0;
                return;
            }
        }

        private void addTextureToCubeTriangles()
        {
            for (int i = 0; i < nTrianglesList; i += 6)
            {
                // Primul triunghi al fetei
                GL.TexCoord2(0.0, 1.0);
